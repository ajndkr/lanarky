...
class ConversationalRetrievalWebsocketConnection(BaseLangchainWebsocketConnection):
    ...
    def _create_chain_executor(
        ...
        async def wrapper(user_message: str):
            if not isinstance(
                chain.combine_docs_chain.llm_chain.llm.callback_manager,
            ...
            for (
                handler
            ) in chain.combine_docs_chain.llm_chain.llm.callback_manager.handlers:
                if isinstance(handler, AsyncWebsocketCallback):
                    chain.combine_docs_chain.llm_chain.llm.callback_manager.remove_handler(
                        handler
                    )
                    break
            chain.combine_docs_chain.llm_chain.llm.callback_manager.add_handler(
